...
const Page = () => {
  ...
  const selectedPengeluaran = pengeluaran.find((leasing) => leasing.id === Number(id));

  const [selectedStatus, setSelectedStatus] = useState(selectedPengeluaran.status);
  const pengeluaranAtas = selectedPengeluaran.batas_atas;
  const pengeluaranBawah = selectedPengeluaran.batas_bawah;
  const pengeluaranDetail = selectedPengeluaran.detail;
  const pengeluaranKode = selectedPengeluaran.kode;
      ...
      <form className="flex flex-col gap-y-8">
        ...
        <div>
          <Textinput
            ...
            register={register}
            className="h-[52px]"
            defaultValue={pengeluaranDetail}
          />
        </div>
        <div>
          <Textinput
